...
import { MoneyFlowState, MoneyFlowEvent } from '@/types/moneyFlow';
...
export default function Chat() {
	...
	const [moneyFlowState, setMoneyFlowState] = useState<MoneyFlowState>({
		sessionTotal: 0,
		events: [],
		lastEventId: ''
	});
		...
		const handleMessage = (event: MessageEvent) => {
			try {
				...
				if (message.type === 'model_configs_info') {
					...
					setMoneyFlowState(prev => ({
						sessionTotal: prev.sessionTotal + costEvent.cost.totalCost,
						events: [...prev.events, costEvent],
						lastEventId: costEvent.id
					}));
				}
			} catch (error) {
	...
	return (
		<div className="size-full flex min-h-0">
			{/* Main Chat Content */}
			<div className="flex-1 flex flex-col min-h-0 text-text-primary">
		...
		<MoneyFlowTracker 
			...
			onReset={() => setMoneyFlowState({ sessionTotal: 0, events: [], lastEventId: '' })}
			onEventClick={(timestamp) => {
				console.log('Cost event clicked:', timestamp);
			}}
		/>
		</div>
	);
