...
export async function selectTemplate({ env, query, availableTemplates, inferenceContext, images, agentId }: SelectTemplateArgs): Promise<TemplateSelection> {
    ...
    const queueCostEvent = async (event: any) => {
        ...
        try {
            console.log('[QUEUE_COST_EVENT] templateSelection - Getting agentStub for agentId:', agentId);
            const agentStub = env.CodeGenObject.get(env.CodeGenObject.idFromName(agentId));
            console.log('[QUEUE_COST_EVENT] templateSelection - Got agentStub:', !!agentStub);
            
            console.log('[QUEUE_COST_EVENT] templateSelection - Calling queueCostEvent on stub');
            await agentStub.queueCostEvent(event);
            console.log('[QUEUE_COST_EVENT] templateSelection - Successfully queued event');
        } catch (error) {
            console.error('[QUEUE_COST_EVENT] templateSelection - ERROR:', {
                error: error,
...
ENTROPY SEED: ${generateSecureToken(64)} - for unique results`;
        ...
        const broadcastCost = async (type: string, data: any) => {
            console.log('[BROADCAST_COST] templateSelection - Called with type:', type, 'hasData:', !!data);
            if (type === 'money_flow_event') {
                console.log('[BROADCAST_COST] templateSelection - Calling queueCostEvent');
